...
name = 'LGG'
pth = os.path.join('..', '..')
pth_in = os.path.join(pth, 'data', 'unzip', f'TCGA-{name}')
...
fn_data = os.path.join(pth_in, f'TCGA-{name}.star_tpm.tsv')
...
fn_clinical = os.path.join(pth_in, f'TCGA-{name}.clinical.tsv')
fn_survival = os.path.join(pth_in, f'TCGA-{name}.survival.tsv')
...
adata_out.write(os.path.join(pth_out, f'TCGA_{name}.h5ad'))
